...
public class TC_22_GeneralKnowledgeQuizTestForAll_102025QueOptions extends BaseClass{
			     ...
			     for(int i=a;i<takeQuiz.size();i++)
			      {
				    ...
				    for(int k=b;k<radio.size();k++)
					{
						...
						test.info("General Knowledge quiz started for "+value+" questions.");
						...
						if(value.equalsIgnoreCase("10"))
						 {
						    ...
						    System.out.println("General Knowledge QUIZ start for 10 question: "+gkquiz);
						    test.info("General Knowledge QUIZ start for 10 question: "+gkquiz);
						    ...
						    try
							{
								...
								if(error==true)
								{
									captureScreenshot(driver,gkquiz+value+"_GK_NoQueFound");
							...
							}catch(Exception ElementNotInteractableException)
							{
								...
								test.info("Quiz started for "+gkquiz+" testtttt Test.");
								...
								for(int j=1;j<=num1-1;j++)
								{
								   ...
								   int numQue = Integer.parseInt(queCount);
								     if(numQue!=10)
								       {
									      captureScreenshot(driver,gkquiz+"_GK_10QueQuizSubmittedFor_"+queCount+"_que");
									      test.fail("General Knowledge Quiz submitted for less than 10 questions. No of question found: "+queCount);
				  				       }
								     else 
								       {
								          test.pass("General Knowledge quiz Test Pass for all "+queCount+" questions")	;
								       }
								     gkQuiz.closeQuizWindow();
								     Thread.sleep(1500);
						...
						if(value.equalsIgnoreCase("20"))
						 {
						    ...
						    Thread.sleep(1500);
						    String gkquiz = quizName.get(i).getText();
						    System.out.println("General Knowledge QUIZ start for 20 question: "+gkquiz);
						    test.info("General Knowledge QUIZ start for 20 question: "+gkquiz);
							...
							}catch(Exception ElementNotInteractableException)
							{
								...
								for(int j=1;j<=num1-1;j++)
								{
								   ...
								   int numQue = Integer.parseInt(queCount);
								     if(numQue!=20)
								       {
									      captureScreenshot(driver,gkquiz+"_GK_20QueQuizSubmittedFor_"+queCount+"_que");
									      test.fail("General Knowledge Quiz submitted for less than 20 questions. No of question found: "+queCount);
				  				       }
								     else 
								       {
						...
						if(value.equalsIgnoreCase("25"))
						 {
						    ...
						    Thread.sleep(1500);
						    String gkquiz = quizName.get(i).getText();
						    System.out.println("General Knowledge QUIZ start for 25 question: "+gkquiz);
						    test.info("General Knowledge QUIZ start for 25 question: "+gkquiz);
							...
							}catch(Exception ElementNotInteractableException)
							{
								...
								for(int j=1;j<=num1-1;j++)
								{
								   ...
								   int numQue = Integer.parseInt(queCount);
								     if(numQue!=25)
								       {
									      captureScreenshot(driver,gkquiz+"_GK_25QueQuizSubmittedFor_"+queCount+"_que");
									      test.fail("General Knowledge Quiz submitted for less than 25 questions. No of question found: "+queCount);
				  				       }
								     else 
								       {
								          test.pass("General Knowledge Test Pass for all "+queCount+" questions")	;
